	...
	m_brake("Brake:", m_agentOrange)
{
	...
	if (!m_carSheetTexture.loadFromFile("./resources/images/CarSprite.png"))
	{
		std::string main("Error Loading Texture");
		throw std::exception(main.c_str());
	}

	if (!m_selectTexture.loadFromFile("./resources/images/arrow.png"))
	{
		std::string main("Error Loading Texture");
		throw std::exception(main.c_str());
	}
	...
	if (!m_garageTexture.loadFromFile("./resources/images/garage.png"))
	{
