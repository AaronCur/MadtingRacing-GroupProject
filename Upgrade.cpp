...
Upgrade::Upgrade(Game & game, sf::Font font) :
	...
	m_comicSans(font)
{
	if (!m_speedTexture.loadFromFile("./resources/images/Speed.png"))
	{
	...
	if (!handlingTexture.loadFromFile("./resources/images/handling.png"))
	{
		std::string s("Error Loading Texture");
		throw std::exception(s.c_str());
	}

	if (!brakeTexture.loadFromFile("./resources/images/brake.png"))
	{
		std::string s("Error Loading Texture");
		throw std::exception(s.c_str());
	}

	if (!speedGaugeTexture.loadFromFile("./resources/images/speedGauge.png"))
	{
		std::string s("Error Loading Texture");
		throw std::exception(s.c_str());
	}
	...
	if (!m_backgroundTexture.loadFromFile("./resources/images/mechanicBackground.png"))
	{
		std::string main("Error Loading Texture");
		throw std::exception(main.c_str());
	}
	...
	if (!m_shader.loadFromFile("upgrade.txt", sf::Shader::Fragment))
	{
...
{
	shaderTime += dt;
	updateShader = shaderTime.asSeconds();
